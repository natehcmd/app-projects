...
final class StatsService: ObservableObject {
    ...
    private func sample() {
        ...
        // Carry forward last known battery/process readings until the background
        // sample below lands, so we don't flash to "unknown" every tick.
        s.topProcesses = stats.topProcesses
        s.batteryPercent = stats.batteryPercent
        s.batteryCharging = stats.batteryCharging
        self.stats = s

        // pmset/ps are real subprocess spawns (fork+exec+text parsing) — expensive
        // enough that running them synchronously on the main actor every second
        // caused visible UI stutter. Both methods are `nonisolated` (they don't
        // touch any @Published state directly), so compute off-thread and only
        // hop back to the main actor to publish the result.
        DispatchQueue.global(qos: .utility).async { [weak self] in
            guard let self else { return }
            var charging = false
            let battery = self.sampleBattery(charging: &charging)
            let processes = self.sampleTopProcesses()
            Task { @MainActor [weak self] in
                guard let self else { return }
                self.stats.batteryPercent = battery
                self.stats.batteryCharging = charging
                self.stats.topProcesses = processes
            }
        }
    ...
    nonisolated private func sampleBattery(charging: inout Bool) -> Double? {
    ...
    nonisolated private func sampleTopProcesses() -> [ProcessRow] {
        let p = Process()
        p.launchPath = "/bin/ps"
