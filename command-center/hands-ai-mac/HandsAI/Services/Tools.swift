...
struct Tools {
    ...
    static func run(toolCall: OllamaClient.ToolCallReq) async -> String {
        ...
        do {
            ...
            case "web_fetch":       return quarantine(try await webFetch(args: args), source: "web_fetch")
            case "web_search":      return quarantine(try await webSearch(args: args), source: "web_search")
    ...
    static func quarantine(_ text: String, source: String = "untrusted") -> String {
        return "<<<UNTRUSTED_DATA_START>>> (source: \(source))\n" +
    ...
    static let runClaudeCLISpec = Spec(function: .init(
        name: "run_claude_cli",
        description: "Delegate a coding/agentic sub-task to the real Claude Code CLI (a full agent with file/bash tools of its own), non-interactively. Use for real repo work — reading/editing multiple files, running tests, git — that's bigger than a single tool call. Takes a while; only use when the task genuinely needs an agentic sub-run, not for a quick question.",
        parameters: .init(properties: [
            "prompt": .init(type: "string", description: "The task to hand off, in full — the CLI has no other context."),
            "cwd": .init(type: "string", description: "Working directory (repo root, etc). Defaults to home."),
        ], required: ["prompt"])
    ))

    static let runAgySpec = Spec(function: .init(
        name: "run_agy",
        description: "Delegate a task to agy (Antigravity, Gemini-backed) — a second independent agent, useful for a deep second-opinion pass or large-context analysis Claude/Ollama can't do as well. Non-interactive, runs with full tool access. Takes a while; use for real deep-dive work, not quick questions.",
        parameters: .init(properties: [
            "prompt": .init(type: "string", description: "The task to hand off, in full — agy has no other context."),
            "cwd": .init(type: "string", description: "Working directory. Defaults to home."),
        ], required: ["prompt"])
    ))
    ...
    // MARK: - Delegating to other CLI agents

    /// GUI apps don't inherit the login shell's PATH — same fix
    /// ClaudeCLIClient.resolvePath() already uses: check common install
    /// spots first, then ask a login shell. Not cached (unlike that class)
    /// since these two tools are called rarely, not once at every app launch.
    private static func resolveBinary(_ name: String, candidates: [String]) async -> String? {
    ...
    private static func runProcess(_ path: String, _ args: [String], cwd: String, timeoutSeconds: Int) async -> String {
        let proc = Process()
        proc.executableURL = URL(fileURLWithPath: path)
        proc.arguments = args
        proc.currentDirectoryURL = URL(fileURLWithPath: expandPath(cwd))
        let outPipe = Pipe()
        proc.standardOutput = outPipe
        proc.standardError = outPipe
        do {
            ...
            return "error: failed to launch \(path) — \(error.localizedDescription)"
        }

        // Same class of problem run_bash sidesteps by not having one: these
        // delegate to a full agentic CLI that can genuinely run for minutes,
        // so waitUntilExit() alone (no timeout) would hang the whole
        // conversation indefinitely on a stuck sub-agent. Race it instead.
        ...
        if !exited {
            proc.terminate()
            return "error: \(path) timed out after \(timeoutSeconds)s"
        }

        let data = outPipe.fileHandleForReading.readDataToEndOfFile()
        var out = String(data: data, encoding: .utf8) ?? "(binary output)"
        if out.utf8.count > 6000 {
            out = String(out.prefix(6000)) + "\n…(truncated)"
        }
        return out.isEmpty ? "(no output, exit \(proc.terminationStatus))" : out
    }

    static func runClaudeCLI(args: Args) async throws -> String {
        guard let prompt = args.string("prompt") else { return "error: missing 'prompt'" }
        let cwd = args.string("cwd") ?? NSHomeDirectory()
        guard let path = await resolveBinary("claude", candidates: [
            "\(NSHomeDirectory())/.local/bin/claude", "/opt/homebrew/bin/claude", "/usr/local/bin/claude",
        ]) else { return "error: claude CLI not found on PATH" }
        // Matches the permission level Command Center's own headless Claude
        // Code calls already use (acceptEdits) — auto-accepts file edits,
        // still asks for anything riskier.
        return await runProcess(path, ["-p", prompt, "--permission-mode", "acceptEdits", "--verbose"],
                                cwd: cwd, timeoutSeconds: 240)
    }

    static func runAgy(args: Args) async throws -> String {
        guard let prompt = args.string("prompt") else { return "error: missing 'prompt'" }
        let cwd = args.string("cwd") ?? NSHomeDirectory()
        guard let path = await resolveBinary("agy", candidates: [
            "/opt/homebrew/bin/agy", "\(NSHomeDirectory())/.local/bin/agy",
        ]) else { return "error: agy CLI not found on PATH" }
        // agy specifically needs --dangerously-skip-permissions in headless
        // mode — without it, any tool use it wants to do (reading files,
        // etc.) blocks on a permission prompt with no one there to answer,
