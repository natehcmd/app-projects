...
final class VoiceService: NSObject, ObservableObject {
    ...
    /// Fires when the TTS queue fully drains — the hands-free conversation
    /// loop uses it to re-arm the mic after the assistant finishes talking.
    var onAllSpeechFinished: (() -> Void)?

    /// Set by AppDelegate to hand the single audio-input tap back and forth
    /// with WakeService. Any path that starts listening (orb tap, mic button,
    /// wake word, hotkey) goes through these, so ambient detection can never
    /// be running at the same time as full recognition.
    var willStartListening: (() -> Void)?
    var didStopListening: (() -> Void)?

    /// Short spoken acknowledgment ("Right away, sir") — queued *before* the
    /// work happens so the latency of the agent loop is covered by voice.
    func speakAck() {
        guard voiceEnabled, !isSpeaking else { return }
        let acks = ["Right away, sir.", "On it, sir.", "One moment.", "Working on it, sir."]
        let utter = AVSpeechUtterance(string: acks.randomElement()!)
        if !selectedVoiceID.isEmpty, let v = AVSpeechSynthesisVoice(identifier: selectedVoiceID) {
            utter.voice = v
        } else {
            utter.voice = Self.pickBestVoice()
        }
    ...
    func startListening() {
        ...
        SFSpeechRecognizer.requestAuthorization { [weak self] status in
            guard status == .authorized else {
                DispatchQueue.main.async { self?.didStopListening?() }
                return
            }
    ...
    private func beginRecognition() {
        guard let recognizer, recognizer.isAvailable else {
            didStopListening?()   // give the mic back to ambient detection
            return
        }
    ...
    func stopListening() {
        ...
        recognitionTask = nil
        isListening = false
        if wasListening { didStopListening?() }
    }
}
...
extension VoiceService: AVSpeechSynthesizerDelegate {
    ...
    nonisolated func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer,
        ...
        Task { @MainActor in
            ...
            // The queue drains one utterance at a time; only the last finish
            // leaves the synthesizer idle. That's when the mic may re-arm.
            if !synthesizer.isSpeaking { self.onAllSpeechFinished?() }
        }
    }
    nonisolated func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer,
