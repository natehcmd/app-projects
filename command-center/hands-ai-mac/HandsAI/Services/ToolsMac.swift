    ...
    static func calendarAddEvent(args: Args) -> String {
        ...
        if granted {
            ...
            do {
                ...
                return "event '\(title)' created for \(start)"
            } catch {
                return "error saving event: \(error.localizedDescription)"
            }
        }

        // Fallback using explicit date components in AppleScript (locale-independent)
        let cal = Calendar(identifier: .gregorian)
        let sY = cal.component(.year, from: startDate), sM = cal.component(.month, from: startDate), sD = cal.component(.day, from: startDate)
        let sH = cal.component(.hour, from: startDate), sMin = cal.component(.minute, from: startDate)
        let eY = cal.component(.year, from: endDate), eM = cal.component(.month, from: endDate), eD = cal.component(.day, from: endDate)
        let eH = cal.component(.hour, from: endDate), eMin = cal.component(.minute, from: endDate)
        let script = """
        tell application "Calendar"
            tell first calendar whose writable is true
                ...
                make new event with properties {summary:"\(escAS(title))", start date:sDate, end date:eDate}
            end tell
        end tell
        """
        let r = osascript(script)
