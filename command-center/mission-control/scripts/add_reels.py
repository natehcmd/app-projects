...
COOKIES = Path.home() / "AgentDrop-Workspace" / ".ig-cookies.txt"
MODEL = Path.home() / "AgentDrop-Workspace" / ".models" / "ggml-base.en.bin"
if not MODEL.exists():
...
def ingest(url):
    ...
    r = sh(["yt-dlp", "-q", "--no-warnings"] + cookie_args + ["-J", clean])
    if r.returncode != 0 or not r.stdout.strip():
        print(f"FAIL meta {rid}"); return
    meta_f.write_text(r.stdout)
    d = json.loads(r.stdout)
    sh(["yt-dlp", "-q", "--no-warnings"] + cookie_args + ["-f", "ba/b", "-x", "--audio-format", "m4a",
        "-o", str(audio), clean])
    tx = ""
    if audio.exists():
