...
const maxEmails = maxFlag !== -1 ? parseInt(args[maxFlag + 1], 10) : (testBatch ? 10 : 500);

// ===== Auth =====
function getAuth() {
  if (!fs.existsSync(CREDENTIALS_PATH)) {
    console.error('Missing credentials.json — run: node src/setup.js');
    process.exit(1);
  }
  if (!fs.existsSync(TOKEN_PATH)) {
    console.error('Missing token.json — run: node src/setup.js');
    process.exit(1);
  }
  ...
  for (const cat of Object.values(prefs.categories)) {
    ...
    if (!label) {
      const created = await gmail.users.labels.create({
        userId: 'me',
        requestBody: { name: cat.label, labelListVisibility: 'labelShow', messageListVisibility: 'show' },
      });
      label = created.data;
    }
...
function extractBody(payload) {
  if (payload.parts) {
    ...
    if (textPart?.body?.data) {
      return Buffer.from(textPart.body.data, 'base64').toString('utf-8').substring(0, 800);
    }
...
async function main() {
  ...
  for (const msg of messages) {
    try {
      ...
      // In test-batch mode, don't apply labels
      if (!testBatch) {
        ...
        if (labelId) {
          ...
          await gmail.users.messages.modify({
            userId: 'me',
            id: msg.id,
            requestBody: { addLabelIds: [labelId], removeLabelIds: removeLabels },
          });
        }
      }
    } catch (e) {
  ...
  // ===== Output =====
  if (testBatch) {
    // JSON output for test-batch mode
    const flat = [];
