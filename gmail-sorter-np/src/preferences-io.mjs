...
export function loadPreferences(dir) {
  ...
  if (!fs.existsSync(filePath)) throw new Error(`preferences.json not found in ${dir}. Run onboarding first.`);
...
export function addCorrection(prefs, { messageId, from, subject, proposed, corrected, reason }) {
  prefs.corrections.push({ id: randomUUID(), date: new Date().toISOString(), messageId: messageId || null, from, subject, proposed, corrected, reason: reason || null });
  prefs.stats.totalCorrected++;
}

export function addTrustedSender(prefs, type, value) {
  const lower = value.toLowerCase();
  if (type === 'domain') { if (!prefs.trustedSenders.domains.includes(lower)) prefs.trustedSenders.domains.push(lower); }
  else if (type === 'email') { if (!prefs.trustedSenders.emails.includes(lower)) prefs.trustedSenders.emails.push(lower); }
}

export function addSpamPattern(prefs, type, value) {
  const map = { domain: 'domains', email: 'emails', subjectPattern: 'subjectPatterns', bodyPattern: 'bodyPatterns' };
  const key = map[type];
  if (!key) throw new Error(`Unknown spam pattern type: ${type}`);
  if (!prefs.spamPatterns[key].includes(value)) prefs.spamPatterns[key].push(value);
}
...
export function pruneCorrections(prefs, maxKeep = 50) {
  ...
  for (const [key, count] of Object.entries(correctionCounts)) {
    if (count >= 3) {
      ...
      if (corrected === 'important' && domain) { addTrustedSender(prefs, 'domain', domain); derived.push({ type: 'trustedDomain', value: domain, count }); }
      else if (corrected === 'spam' && domain) { addSpamPattern(prefs, 'domain', domain); derived.push({ type: 'spamDomain', value: domain, count }); }
    }
  }
